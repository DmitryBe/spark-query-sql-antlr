...
class TestSqlQueryParser extends FlatSpec with Matchers{
  ...
  shouldPass("select a from dummy as t0 union select b from fake t1")
  ...
  shouldPass("select vcf.*, cv.highest_clinical_significance, cv.hgvs, cv.rs_id, snpf.annotation_impact, snpf.feature_type, snpf.feature_id from vcf.samples as vcf left join annotation.clinvar_rollup cv on (vcf.chromosome = cv.chromosome and vcf.position = cv.position and vcf.ref = cv.ref and vcf.alt = cv.alt) left join annotation.cadd_flatten cadd on (vcf.chromosome = cadd.chromosome and vcf.position = cadd.position and vcf.ref = cadd.ref and vcf.alt = cadd.alt) left join annotation.snpeff_flatten snpf on (vcf.chromosome = snpf.chromosome and vcf.position = snpf.position and vcf.ref = snpf.ref and vcf.alt = snpf.alt) where sample_id = 176444255")
  ...
  def shouldPass(sql: String): Unit ={
    it should sql in {
      val t = SqlQueryParser.parse(sql)
